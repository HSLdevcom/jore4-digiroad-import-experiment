DROP TABLE IF EXISTS digiroad.dr_links;

CREATE TABLE digiroad.dr_links AS
SELECT
    src.gid AS gid,
...
UPDATE digiroad.dr_links SET geom_dump = ST_SetSRID(geom_dump, 3067);

SELECT AddGeometryColumn('digiroad', 'dr_links', 'geom', 3067, 'LINESTRING', 3);
UPDATE digiroad.dr_links SET geom = ST_Force3D(geom_dump);
ALTER TABLE digiroad.dr_links ALTER COLUMN geom SET NOT NULL;

ALTER TABLE digiroad.dr_links DROP COLUMN geom_dump;

ALTER TABLE digiroad.dr_links ADD COLUMN geog geography(LINESTRINGZ, 4326);
UPDATE digiroad.dr_links SET geog = Geography(ST_Transform(geom, 4326));
ALTER TABLE digiroad.dr_links ALTER COLUMN geog SET NOT NULL;

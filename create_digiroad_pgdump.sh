...
docker run --name $DOCKER_CONTAINER_NAME -e POSTGRES_PASSWORD= -d $DOCKER_IMAGE_NAME
...
SHP_ENCODING="UTF-8"
TABLE_INPUT="import.dr_links_in"
TABLE_OUTPUT="import.dr_links"
PGDUMP_OUTPUT="dr_links_$(date "+%Y-%m-%d").pgdump"

docker exec "${DOCKER_CONTAINER_NAME}" sh -c "$PSQL -nt -c 'CREATE SCHEMA import;'"
